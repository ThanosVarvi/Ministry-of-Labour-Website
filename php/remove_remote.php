		...
		if (!empty($_POST)) {
			...
			else
			{
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					...
					window.location.href='./remove_remoteworking.php';
					</SCRIPT>");
			...
			if ($result->num_rows > 0)
			{
				...
				if(isset($_POST['AFM'])&& '' !== $_POST['AFM']&&isset($_POST['name'])&& '' !== $_POST['name']&&isset($_POST['surname'])&& '' !== $_POST['surname']&&isset($_POST['email'])&& '' !== $_POST['email'])
				{
					$sql = "UPDATE users SET start_remote=NULL WHERE id='".$_POST["AFM"]."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Αναστολή Εξ Αποστάσεως Εργασίας')
						window.location.href='./remove_remoteworking.php';
						</SCRIPT>");
					}
					
					$sql = "UPDATE users SET end_remote=NULL WHERE id='".$_POST["AFM"]."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Αναστολή Εξ Αποστάσεως Εργασίας')
						window.location.href='./remove_remoteworking.php';
				...
				else
				{
					...
					$sql = "UPDATE users SET start_remote=NULL WHERE id='".$int_id."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Αναστολή Εξ Αποστάσεως Εργασίας')
						window.location.href='./remove_remoteworking.php';
					...
					$sql = "UPDATE users SET end_remote=NULL WHERE id='".$int_id."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Αναστολή Εξ Αποστάσεως Εργασίας')
						window.location.href='./remove_remoteworking.php';
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('Επιτυχής Αναστολή Εξ Αποστάσεως Εργασίας')
					window.location.href='./remove_remoteworking.php';
					</SCRIPT>");
