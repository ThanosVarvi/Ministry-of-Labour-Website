...
// Always start this first
		...
		//	$user = $result->fetch_object();
			...
			else
			{
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					...
					window.location.href='./remoteworking.php';
					</SCRIPT>");
			...
			if ($result->num_rows > 0)
			{
				...
				if(isset($_POST['AFM'])&& '' !== $_POST['AFM']&&isset($_POST['name'])&& '' !== $_POST['name']&&isset($_POST['surname'])&& '' !== $_POST['surname']&&isset($_POST['email'])&& '' !== $_POST['email'])
				{
					$sql = "UPDATE users SET start_remote='".$start_date."' WHERE id='".$_POST["AFM"]."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Δήλωση Εξ Αποστάσεως Εργασίας!')
						window.location.href='./remoteworking.php';
						</SCRIPT>");
						die('owow');
				
					}
					$sql = "UPDATE users SET end_remote='".$end_date."' WHERE id='".$_POST["AFM"]."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Δήλωση Εξ Αποστάσεως Εργασίας!')
						window.location.href='./remoteworking.php';
				...
				{
					$sql = "UPDATE users SET start_remote='".$start_date."' WHERE id='".$int_id."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Δήλωση Εξ Αποστάσεως Εργασίας!')
						window.location.href='./remoteworking.php';
					...
					$sql = "UPDATE users SET end_remote='".$end_date."' WHERE id='".$int_id."'";
					if ($db_conn->query($sql) != TRUE) {
						echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Δήλωση Εξ Αποστάσεως Εργασίας!')
						window.location.href='./remoteworking.php';
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('Επιτυχής Δήλωση Εξ αποστάσεως Εργασίας!')
					window.location.href='./remoteworking.php';
					</SCRIPT>");
				die('owow');
			...
			{
				echo ("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Μη Επιτυχής Δήλωση Εξ αποστάσεως Εργασίας!Δώθηκαν λανθασμένα στοιχεία')
						window.location.href='./remoteworking.php';
						</SCRIPT>");
				die('owow');
			}
		...
		$_POST=array();
		echo ("<SCRIPT LANGUAGE='JavaScript'>
				window.alert('Μη Επιτυχής Δήλωση Εξ αποστάσεως Εργασίας!Δε δηλώθηκαν στοιχεία.')
				window.location.href='./remoteworking.php';
				</SCRIPT>");
